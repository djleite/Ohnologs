...
use strict;
use warnings;
use Parallel::ForkManager;  # Import the Parallel::ForkManager module
...
my $totalOrganisms = 1; # This should be total organisms to be combined - 1 (as the starting organism will be the first one)

my @allorgs = ('CARROT', 'TACGIG');

# Number of parallel processes you want to run
my $max_processes = 30;  # Adjust this based on your system's capacity
...
my @organisms = ('CARROT', 'TACGIG'); # this array will have all the organisms that I need to add to the ohnolog dataset
...
foreach my $org2add (@organisms){
	...
	if ($org2add ne $organism){ # If it's not the same organism
		...
		else {$outFile = "$organism"."$done\.txt";}my $max_processes = 30;  # Adjust this based on your system's capacity
