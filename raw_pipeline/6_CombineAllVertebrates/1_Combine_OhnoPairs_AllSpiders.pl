...
my $totalOrganisms = 4; # This should be total organisms to be combined - 1 (as the starting organism will be the first one)

my @allorgs = ('ECTDAV', 'DYSSIL','PARTEP','OEDGIB','ULODIV');
...
my @organisms = ('ECTDAV', 'DYSSIL','PARTEP','OEDGIB','ULODIV'); # this array will have all the organisms that I need to add to the ohnolog dataset
...
foreach my $org2add (@organisms){
	...
	if ($org2add ne $organism){ # If it's not the same organism
		...
		while (<CONSENSUS>){
			...
			if (/^(ECTDAV|DYSSIL|PARTEP|OEDGIB|ULODIV).+/){
				print OUT join ("\t", @line),"\t";
			}
			else {
				my $head = $_;
