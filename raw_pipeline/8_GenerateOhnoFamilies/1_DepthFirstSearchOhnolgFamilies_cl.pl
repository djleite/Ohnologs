...
if ((scalar @ARGV) != 2){
	...
	Example usage: perl 1_DepthFirstSearchOhnolgFamilies_cl.pl drerio A\n\n";
	print "Check parameters. Criteria can only be 0, A, B or C. Organism name must be one of the following\n";
	print "@allorgs\n\n";
	exit;
}

print "> Processing $subGenomeName criteria $criteria\n";

# --------------------------------------------------------------------------------------------------------------------
my $subGenomeFile = "../gene_order/$subGenomeName\_gene_order.tsv";
#my $subGenomeFile = '../1_All_Genes/3_Prepare_final_gene_files_all-scaffolds/AllGenes_'.$subGenomeName.'_NCBI.txt'; ## For N furzeri

my $polyploidChrGenes = "../3_SyntenyOutputFiles/$subGenomeName\/self/".$subGenomeName.'_Chromosomes-Genes.txt';
my $outFile = $subGenomeName.'/Families_Criteria-['.$criteria.']_'."$subGenomeName".".txt";
my $pairFile = "../7_FilterOhnologs/$subGenomeName/$subGenomeName\_Criteria-\[$criteria\]-Pairs.txt";
