...
foreach my $polyploidName (@allorgs){
	...
	foreach my $outgroupName (@outgroups){ # Foreach organism ______________________________________________
		...
		open OUT, ">$polyploidName\/$outgroupName-$polyploidName\_CombinedPairs_AllWindows.txt" or die $!;

		foreach (<..\/3_SyntenyOutputFiles\/$polyploidName\/outgroup/$outgroupName*$polyploidName\_OhnoPairs*>){ # Folder name and path
