...
lapTime("PoliploydGeneCount built");
...
sub lapTime {
	...
	print "...[$label - lap time]: ";
	if (eval (time - $lapTime) >= 3600){printf "%.2f", eval(time - $lapTime)/3600; print " hours\n";}
	elsif (eval (time - $lapTime) >= 60){printf "%.2f", eval(time - $lapTime)/60; print " minutes\n";}
	else {print time - $lapTime, " seconds\n";}
